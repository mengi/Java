...
<body>
<%
	session.setAttribute("secilenIslem", "DuyuruDuzenle_Duyurular");
	
	database_fakulteYoneticisiController controller 
								= new database_fakulteYoneticisiController();
	int fakulteID = Integer.parseInt(session.getAttribute("gorevID")+"");
		...
		if (DuyuruBilgi.getDuyuruTurID() == 3){
			ozelDuyuru = "bold";
				...
				<tr style="font-weight: <%=ozelDuyuru%>; ">	
					...
					<td>
						<a class="btn" onclick="autoSubmit('Duzenle<%=i%>')">
							<i class="icon-edit"></i>
						</a>
					</td>
				</tr><%
